...
{
    public class ParametricasRenacRepository : IParametricasRenacRepository
    {
        private readonly DapperContext _context;
        private const string _schema = "renac";

        public ParametricasRenacRepository(DapperContext context)
        {
            _context = context;
        }

        public Response Insert(ParametricasRenac entidad)
        {
            ...
            try
            {
                ...
                {
                    var query = $"{_schema}.usp_parametricas_renac_insertar";

                    var parameters = new DynamicParameters();
                    parameters.Add("p_idpadre", entidad.idPadre);
                    parameters.Add("p_idgrupo", entidad.idGrupo);
        ...
        public Response Update(ParametricasRenac entidad)
        {
            Response retorno = new Response();
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    var query = $"{_schema}.usp_parametricas_renac_actualizar";

                    var parameters = new DynamicParameters();
                    parameters.Add("p_idparametricasrenac", entidad.idParametricasRenac);
                    parameters.Add("p_idpadre", entidad.idPadre);
                    parameters.Add("p_idgrupo", entidad.idGrupo);
                    parameters.Add("p_codigo", entidad.codigo);
                    parameters.Add("p_descripcion", entidad.descripcion);
        ...
        public Response Delete(ParametricasRenac entidad)
        {
            Response retorno = new Response();
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    var query = $"{_schema}.usp_parametricas_renac_eliminar";
        ...
        public Response<dynamic> GetById(ParametricasRenac entidad)
        {
            ...
            try
            {
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    var result = sqlConnection.QuerySingle<ParametricasRenac>($"SELECT * FROM {_schema}.fn_parametricas_renac_seleccionar({entidad.idParametricasRenac})");
        ...
        public Response<List<dynamic>> GetList(ParametricasRenac entidad)
        {
            ...
            try
            {
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    {
                        var command = new NpgsqlCommand($"{_schema}.usp_parametricas_renac_seleccionar", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;

                        command.Parameters.Add("@p_idparametricasrenac", NpgsqlDbType.Integer).Value = entidad.idParametricasRenac == null ? 0 : (int)entidad.idParametricasRenac;
        ...
        public Response<List<dynamic>> GetListPaginated(ParametricasRenac entidad, out int PageSize, out int PageNumber, out int TotalReg)
        {
            Response<List<dynamic>> retorno = new Response<List<dynamic>>();
            int _PageSize = 0;
            int _PageNumber = 0;
            ...
            try
            {
                ...
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    using (var tran = sqlConnection.BeginTransaction())
                    {
                        var command = new NpgsqlCommand($"{_schema}.usp_parametricas_renac_paginado", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;
