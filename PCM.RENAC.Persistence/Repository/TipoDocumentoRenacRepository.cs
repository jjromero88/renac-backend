...
{
    public class TipoDocumentoRenacRepository : ITipoDocumentoRenacRepository
    {
        private readonly DapperContext _context;
        private const string _schema = "renac";

        public TipoDocumentoRenacRepository(DapperContext context)
        {
            _context = context;
        }

        public Response Insert(TipoDocumentoRenac entidad)
        {
            ...
            try
            {
                ...
                {
                    var query = $"{_schema}.usp_tipo_documento_renac_insertar";
        ...
        public Response Update(TipoDocumentoRenac entidad)
        {
            Response retorno = new Response();
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    var query = $"{_schema}.usp_tipo_documento_renac_actualizar";

                    var parameters = new DynamicParameters();
                    parameters.Add("p_idtipodocumentorenac", entidad.idTipoDocumentoRenac);
                    parameters.Add("p_codigo", entidad.codigo);
                    parameters.Add("p_descripcion", entidad.descripcion);
        ...
        public Response Delete(TipoDocumentoRenac entidad)
        {
            Response retorno = new Response();
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    var query = $"{_schema}.usp_tipo_documento_renac_eliminar";
        ...
        public Response<dynamic> GetById(TipoDocumentoRenac entidad)
        {
            ...
            try
            {
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    var result = sqlConnection.QuerySingleOrDefault<dynamic>($"SELECT * FROM {_schema}.fn_tipo_documento_renac_seleccionar({entidad.idTipoDocumentoRenac})");
        ...
        public Response<List<dynamic>> GetList(TipoDocumentoRenac entidad)
        {
            ...
            try
            {
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    {
                        var command = new NpgsqlCommand($"{_schema}.usp_tipo_documento_renac_seleccionar", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;

                        command.Parameters.Add("@p_idtipodocumentorenac", NpgsqlDbType.Integer).Value = entidad.idTipoDocumentoRenac == null ? 0 : (int)entidad.idTipoDocumentoRenac;
        ...
        public Response<List<dynamic>> GetListPaginated(TipoDocumentoRenac entidad, out int PageSize, out int PageNumber, out int TotalReg)
        {
            Response<List<dynamic>> retorno = new Response<List<dynamic>>();
            int _PageSize = 0;
            int _PageNumber = 0;
            ...
            try
            {
                ...
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    using (var tran = sqlConnection.BeginTransaction())
                    {
                        var command = new NpgsqlCommand($"{_schema}.usp_tipo_documento_renac_paginado", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;
