...
{
    public class InformeDerivacionRepository : IInformeDerivacionRepository
    {
        private readonly DapperContext _context;
        private const string _schema = "renac";

        public InformeDerivacionRepository(DapperContext context)
        {
            _context = context;
        }

        public Response Insert(InformeDerivacion entidad)
        {
            ...
            try
            {
                ...
                {
                    var query = $"{_schema}.usp_informe_derivacion_insertar";
        ...
        public Response Update(InformeDerivacion entidad)
        {
            Response retorno = new Response();
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    var query = $"{_schema}.usp_informe_derivacion_actualizar";

                    var parameters = new DynamicParameters();
                    parameters.Add("p_idinformederivacion", entidad.idInformeDerivacion);
                    parameters.Add("p_idinformerenac", entidad.idInformeRenac);
                    parameters.Add("p_idderivacionrenac", entidad.idDerivacionRenac);              
        ...
        public Response Delete(InformeDerivacion entidad)
        {
            Response retorno = new Response();
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    var query = $"{_schema}.usp_informe_derivacion_eliminar";
        ...
        public Response<dynamic> GetById(InformeDerivacion entidad)
        {
            ...
            try
            {
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    var result = sqlConnection.QuerySingleOrDefault<dynamic>($"SELECT * FROM {_schema}.fn_informe_derivacion_seleccionar({entidad.idInformeDerivacion})");
        ...
        public Response<List<dynamic>> GetList(InformeDerivacion entidad)
        {
            ...
            try
            {
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    {
                        var command = new NpgsqlCommand($"{_schema}.usp_informe_derivacion_seleccionar", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;

                        command.Parameters.Add("@p_idinformederivacion", NpgsqlDbType.Integer).Value = entidad.idInformeDerivacion == null ? 0 : (int)entidad.idInformeDerivacion;
        ...
        public Response<List<dynamic>> GetListPaginated(InformeDerivacion entidad, out int PageSize, out int PageNumber, out int TotalReg)
        {
            Response<List<dynamic>> retorno = new Response<List<dynamic>>();
            int _PageSize = 0;
            int _PageNumber = 0;
            ...
            try
            {
                ...
                using (var sqlConnection = new NpgsqlConnection(connection.ConnectionString))
                {
                    ...
                    using (var tran = sqlConnection.BeginTransaction())
                    {
                        var command = new NpgsqlCommand($"{_schema}.usp_informe_derivacion_paginado", sqlConnection);
                        command.CommandType = CommandType.StoredProcedure;
